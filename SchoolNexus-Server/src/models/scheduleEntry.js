...
export async function createScheduleEntry(scheduleEntryObj = {}) {
    ...
    if (scheduleEntryObj.tcaId === "" || scheduleEntryObj.tcaId === undefined) {
        ...
        if (tcaIds.length === 0) {
            if (process.env.VERBOSITY >= 1) {
                console.error(
                    "Failed to find a TCA to assign to scheduleEntry"
                );
            }
            return false;
        } else {
    ...
    if (
        scheduleEntryObj.scheduleId === "" ||
        scheduleEntryObj.scheduleId === undefined
    ) {
        // Get the schoolId of the teacher assigned to the TCA
        const teacherId = await pint.find(
            "teacherClasssAssignment",
            { teacherId: true },
            { id: scheduleEntryObj.tcaId },
            true
        )[0];
        ...
        const scheduleIds = await pint.find(
        ...
        while (!success && retries > 0) {
            ...
            if (
                scheduleEntryObj.dayOfWeek === "" ||
                scheduleEntryObj.dayOfWeek === undefined
            ) {
                scheduleEntryObj.dayOfWeek = chance.integer({ min: 1, max: 5 });
            } else if (
                scheduleEntryObj.dayOfWeek < 1 ||
                scheduleEntryObj.dayOfWeek > 7
            ) {
                if (process.env.VERBOSITY >= 1) {
            ...
            if (
                scheduleEntryObj.timeSlot === "" ||
                scheduleEntryObj.timeSlot === undefined
            ) {
                scheduleEntryObj.timeSlot = chance.integer({ min: 1, max: 8 });
            } else if (
                scheduleEntryObj.timeSlot < 1 ||
                scheduleEntryObj.timeSlot > 8
            ) {
                if (process.env.VERBOSITY >= 1) {
            ...
            const sameDowTsTeacherIds = await pint.find(
                "teacherClasssAssignment",
                { teacherId: true },
                { id: { in: sameDowTsTcaIds } },
                true
            );
            ...
            if (sameDowTsTeacherIds.length > 0) {
                if (sameDowTsTeacherIds.includes(teacherId) === true) {
                    if (process.env.VERBOSITY >= 1) {
                        console.error(
                            "Teacher already has a schedule entry at the same time"
                        );
                    }
                    retries--;
                    continue;
        ...
        (await pint.find(
            "schoolQuarteralSchedule",
            { id: true },
            { id: scheduleEntryObj.scheduleId },
            true
        ).length) === 0
    ) {
        if (process.env.VERBOSITY >= 1) {
...
export async function createScheduleEntriesFromTemplate(
    scheduleEntryTemplate = {},
    numOfScheduleEntries = null
) {
    if (numOfScheduleEntries === null) {
        ...
        for (const tcaId of tcaIds) {
            ...
            let retries = 5;
            while (!success && retries > 0) {
                success = await createScheduleEntry({
                    ...scheduleEntryTemplate,
                    tcaId,
                });
                retries--;

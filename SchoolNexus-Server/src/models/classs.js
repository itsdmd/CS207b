...
export async function generateSequentialClasssName(schoolId, grade = null) {
	...
	} else if (!(await pint.find("school", { id: true }, { id: schoolId }, false))) {
		console.error("schoolId not found: " + schoolId);
		return false;
	}

	// Get school's gradeLevels
	const gradeLevels = await pint.find("school", { gradeLevels: true }, { id: schoolId }, true);
...
export async function createClasss(classsObj = {}) {
	...
	try {
		...
		console.error("Failed to create classs " + classsObj.name + ": " + error);
		return false;
	}
}
...
export async function createClassses(classsObjs = []) {
	if (classsObjs.length === 0) {
		...
		for (const schoolId of schoolIds) {
			for (let i = 1; i <= 12; i++) {
				await createClasss({ schoolId: schoolId, name: i.toString() + "A1" });
			}
		}
	} else {
...
export async function createClasssesFromTemplate(classsTemplate = {}, numClassses = 1) {
	if (numClassses <= 0) {

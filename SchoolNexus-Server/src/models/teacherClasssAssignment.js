...
export async function createTeacherClasssAssignment(tcaObj = {}) {
    ...
    if (tcaObj.classsId === "" || tcaObj.classsId === undefined) {
        ...
        const classsWithFormTeacherIds = await pint.find(
            "classs",
            { id: true },
            { members: { some: { accountType: "TEACHER" } } },
            true
        );
        ...
        if (classsIds.length === 0) {
            if (process.env.VERBOSITY >= 1) {
                console.error(
                    "Failed to find a class to assign to teacherId " +
                        tcaObj.teacherId
                );
            }
            return false;
        ...
        (await pint.find("classs", { id: true }, { id: tcaObj.classsId }, true)
            .length) === 0
    ) {
        if (process.env.VERBOSITY >= 1) {
    ...
    const currentTeachers = pint.find(
    ...
    if (tcaObj.teacherId === "" || tcaObj.teacherId === undefined) {
        try {
                ...
                if (
                    (
                        await pint.find(
                            "teacherClasssAssignment",
                            { id: true },
                            { teacherId: teacherId, classsId: tcaObj.classsId },
                            true
                        )
                    ).length > 0
                ) {
            ...
            if (teacherIds.length === 0) {
                if (process.env.VERBOSITY >= 1) {
                    console.error(
                        "Failed to find a teacher to assign to classsId " +
                            tcaObj.classsId +
                            ". Error: " +
                            error
                    );
        ...
        (await pint.find("user", { id: true }, { id: tcaObj.teacherId }, true)
            .length) === 0
    ) {
        if (process.env.VERBOSITY >= 1) {
        ...
        if (
            ...
            if (process.env.VERBOSITY >= 2) {
                console.warn(
                    "teacherId " +
                        tcaObj.teacherId +
                        " is already assigned to classsId " +
                        tcaObj.classsId
                );
            }
            return false;
        ...
        // or is not in the same school as this class
            ...
            if (process.env.VERBOSITY >= 2) {
                console.warn(
                    "teacherId " +
                        tcaObj.teacherId +
                        " is not in the same school as classsId " +
                        tcaObj.classsId
                );
            }
            return false;
    ...
    try {
        ...
        if (process.env.VERBOSITY >= 1) {
            console.error(
                "Failed to create TCA for " + tcaObj.teacherId + ": " + error
            );
        }
        return false;
    }
...
export async function createTeacherClasssAssignments(tcaObjs = []) {
    if (tcaObjs.length === 0) {
        ...
        if (classsWithFormTeacherIds.length === 0) {
            if (process.env.VERBOSITY >= 2) {
                console.warn("No classes with form teachers.");
            }
        } else {
            // Assign the form teachers to their respective classes
            for (const classsId of classsWithFormTeacherIds) {
                const formTeacherIds = await pint.find(
                    "user",
                    { id: true },
                    { classsId: classsId, accountType: "TEACHER" },
                    true
                );
                for (const teacherId of formTeacherIds) {
                    const status = await createTeacherClasssAssignment({
                        teacherId: teacherId,
                        classsId: classsId,
                    });
        ...
        const teacherIds = await pint.find(
            "user",
            { id: true },
            { accountType: "TEACHER", id: { notIn: formTeachers } },
            true
        );

        if (teacherIds.length === 0) {
            if (process.env.VERBOSITY >= 2) {
                console.warn("No teachers to assigned to classes.");
            }
            return false;
        }
        ...
        for (const teacherId of teacherIds) {
            ...
            while (!success && retries < 10) {
                success = await createTeacherClasssAssignment({
                    teacherId: teacherId,
                });
                retries++;
            }

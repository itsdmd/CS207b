...
export async function createGradeType(gradeTypeObj = {}) {
    ...
    if (gradeTypeObj.name === null || gradeTypeObj.name === undefined) {
        ...
        gradeTypeObj.multiplier === null ||
        gradeTypeObj.multiplier === undefined
    ) {
        if (process.env.VERBOSITY >= 1) {
    ...
    if (
        gradeTypeObj.shortName === null ||
        gradeTypeObj.shortName === undefined
    ) {
        gradeTypeObj.shortName = gradeTypeObj.name.toLowerCase();
        gradeTypeObj.shortName =
            gradeTypeObj.shortName.charAt(0).toUpperCase() +
            gradeTypeObj.shortName.slice(1);
    }
    ...
    if (
        (
            await pint.find(
                "gradeType",
                { id: true },
                { schoolId: gradeTypeObj.schoolId, name: gradeTypeObj.name },
                true
            )
        ).length > 0
    ) {
    ...
    try {
        ...
        if (process.env.VERBOSITY >= 1) {
            console.error(
                "Failed to create grade type " + gradeTypeObj.name + ": " + err
            );
        }
        return false;
    }
...
export async function populateDefaultGradeTypes() {
    ...
    for (let i = 0; i < TYPES_SHORT.length; i++) {
        const gradeTypeObj = {
            shortName: TYPES_SHORT[i],
            name: TYPES_FULL[i],
            multiplier: TYPES_MULTIPLIER[i],
        };

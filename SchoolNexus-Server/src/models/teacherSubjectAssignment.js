...
export async function createTeacherSubjectAssignment(tsaObjs = {}) {
	if (tsaObjs.subjectId === "" || tsaObjs.subjectId === undefined) {
		...
		if (subjectIds.length === 0) {
			console.error("Failed to find a subject to assign to teacherId " + tsaObjs.teacherId);
			return false;
		} else {
			tsaObjs.subjectId = chance.pickone(subjectIds);
		}
	} else if (!(await pint.find("subject", { id: true }, { id: tsaObjs.subjectId }, false))) {
	...
	if (tsaObjs.teacherId === "" || tsaObjs.teacherId === undefined) {
		...
		const assignedTeacherIds = await pint.find("teacherSubjectAssignment", { teacherId: true }, { teacherId: { notIn: teacherIds } }, true);

		if (assignedTeacherIds.length === 0) {
			console.error("Failed to find a teacher to assign to subjectId " + tsaObjs.subjectId);
			return false;
		} else {
			tsaObjs.teacherId = chance.pickone(assignedTeacherIds);
		}
	} else if (!(await pint.find("user", { id: true }, { id: tsaObjs.teacherId }, false))) {
	...
	try {
		...
		console.log("Created TSA for " + tsaObjs.teacherId + ": " + tsaObjs.subjectId);
		return true;
	} catch (error) {
		console.error("Failed to create TSA for " + tsaObjs.teacherId + ": " + error);
		return false;
	}
}
...
export async function createTeacherSubjectAssignmentsFromTemplate(tsaTemplate = {}, numTsa = 1) {
	if (numTsa <= 0) {
